...
$userList = array();
$databash = array();
$webdata = get_url_phone("http://zs.hnvist.cn/column/notice/details/1600758017560.shtml");
...
foreach ($list[1] as $value) {
    ...
    $userinfo = ['offer_id' => $infos[1][1] .'_0'. $infos[1][0], 'user_name' => $infos[1][2], 'user_sex' => $infos[1][2], 'user_sex' => $infos[1][3], 'identity_end' => $infos[1][4], 'courier_id' => $infos[1][5]];
    array_push($userList, $userinfo);
    // print_r(json_encode($userinfo, JSON_UNESCAPED_UNICODE) . PHP_EOL);
}

set_file_text("./data_04.json", json_encode($userList, JSON_UNESCAPED_UNICODE));
print_r(PHP_EOL);
